...
Int main(Int argc, const char* argv[])
{
    ...
    //stati iniziali
    StringConstant startHeating("startHeating"); //rooms
    StringConstant tmp("tmp"); //clock
    StringConstant start("start"); //valvola
    ...
    HybridSet initial_set({heating|startHeating, time|tmp, heating2|startHeating, valve|start},{Tinitmin<=temp<=Tinitmax, t==0,Tinitmin<=temp2<=Tinitmax });
    ...
    plot("FireHotRoom",time_temp2_axes, Colour(0.0,0.5,1.0), orbit);
