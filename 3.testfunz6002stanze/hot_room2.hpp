...
inline CompositeHybridAutomaton create_heating_system()
{
    ...
    hr.new_mode( heating|start, {dot(temp) = 140*pow(t,Nat(0.25))+20} );
    hr.new_mode( heating|end, {dot(temp) = a*((log(b*(t-d)+c))/log(r))+d} );
    hr.new_mode( heating|coolon, {dot(temp) = (-g)*(t-timef*(i))+tempf});
    hr.new_mode( heating|coolgoon, {dot(temp) = (-l)*(h-timef)*(t-timef*(i))+tempf});
    hr.new_mode( heating|cooloff, {dot(temp) = (-l)*(t-timef*(i))+tempf});
