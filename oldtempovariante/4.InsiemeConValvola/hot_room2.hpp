...
inline HybridAutomaton create_heating_system2()
{
    ...
    RealConstant h("h",4.1_decimal); // costante per coolgoon no valvola
    ...
    hr2.new_mode( heating2|coolon, {dot(temp2) = tempf+((-g)*(aperture2)*(timef-t*(-i)))});
    //hr2.new_mode( heating2|coolon, {dot(temp2) = tempf+((-g)*(aperture2)*(timef-t*(-i/aperture2)))});
    hr2.new_mode( heating2|coolgoon, {dot(temp2) = tempf+((-l)*(aperture2)*(h-t)*(timef-t*(-u/aperture2)))});
    hr2.new_mode( heating2|cooloff, {dot(temp2) = tempf+((-l)*(aperture2)*(timef-t*(-s/aperture2)))});
