...
inline HybridAutomaton getValve()
{
    ...
    valve_automaton.new_mode(valve|start,{let(aperture)=0});
    valve_automaton.new_mode(valve|firer2,{let(aperture)=1});
    valve_automaton.new_mode(valve|end, {let(aperture)=0});
    ...
    valve_automaton.new_mode(valve|modulated,{dot(aperture)=K*(temp-temp2)});

    // Define the transitions
    valve_automaton.new_transition(valve|start,fire_room2,valve|firer2,next(aperture)=aperture,temp2>=tempfire,EventKind::URGENT);
    valve_automaton.new_transition(valve|firer2,fire_room1,valve|modulated,{next(aperture)=aperture},temp>=tempfire,EventKind::URGENT);
    valve_automaton.new_transition(valve|modulated,closing,valve|end,{next(aperture)=aperture},temp<tempmin,EventKind::URGENT);
