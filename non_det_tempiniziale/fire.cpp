...
Int main(Int argc, const char* argv[])
{
    ...
    RealConstant init("init", 19.75_decimal);
    ...
    Real Tinitmin(Ti+r); Real Tinitmax(Ti+3*r); Real Cinitmin(0+r);  // Tinit=16.0;
    HybridSet initial_set(heating|startHeating,{temp==init, 0<=t<=Cinit_max});
    std::cout << "t: " << t << " temp: " << temp << std::flush;
