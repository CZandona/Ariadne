...
inline CompositeHybridAutomaton create_heating_system()
{
    ...
    RealConstant r("r",20_decimal); // costante per cooloff
    RealConstant ts("ts",0.01_decimal); // costante per cooloff
    ...
    hr.new_invariant( heating|startHeating, t<=ts+delta, must_heating );
    hr.new_transition( heating|startHeating, on, heating|start, {next(temp)=temp}, t>=ts-delta, EventKind::PERMISSIVE );
