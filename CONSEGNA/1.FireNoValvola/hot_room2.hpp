...
inline HybridAutomaton create_heating_system2()
{
    ...
    RealConstant g("g",450); // costante per coolon 
    ...
    RealConstant u("u",1.10_decimal); // fattore correttivo per coolon e coolgoon
    RealConstant v("v",1.90_decimal); // fattore correttivo per cooloff
    ...
    DiscreteEvent more_hot2("more_hot2"); // evento da start a end
    DiscreteEvent too_hot2("too_hot2"); // evento da end a coolon
    DiscreteEvent less_hot2("less_hot2"); // evento da coolon a coolgoon
    DiscreteEvent less_less_hot2("less_less_hot2"); // evento da coolgoon a cooloff
    DiscreteEvent end_cool2("end_cool2"); // evento da cooloff a stopHeating

    
    
    hr2.new_mode( heating2|start, {dot(temp2) = 700*pow(t,Nat(0.25))+200} );
    hr2.new_mode( heating2|end, {dot(temp2) = a*((log(b*(d-ts)+c))/log(r))+ts} );
    hr2.new_mode( heating2|coolon, {dot(temp2) = tempf+((-g)*(timef-(timef+0.01_decimal)*(-u)))});
    hr2.new_mode( heating2|coolgoon, {dot(temp2) = tempf+((-l)*(h-(timef+p)))*(timef-(timef+p)*(-u))});
    hr2.new_mode( heating2|cooloff, {dot(temp2) = tempf+((-l)*(timef-(timef+p+q)*(-v)))});
