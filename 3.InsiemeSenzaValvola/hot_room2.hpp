...
inline HybridAutomaton create_heating_system2()
{
    ...
    RealConstant h("h",4.1_decimal); // costante per coolgoon
    ...
    //hr2.new_mode( heating2|start, {dot(temp2) = 154*pow(t,0.25_decimal)+20} );
    hr2.new_mode( heating2|start, {dot(temp2) = 154*pow(t,Nat(0.25))+20} );
    hr2.new_mode( heating2|end, {dot(temp2) = a*((log(b*(t-d)+c))/log(r))+d} );
    hr2.new_mode( heating2|coolon, {dot(temp2) = tempf+((-g)*(timef-t*(-i)))});
    ...
    hr2.new_mode( heating2|coolgoon, {dot(temp2) = tempf+((-l)*(h-t)*(timef-t*(-u)))});
    hr2.new_mode( heating2|cooloff, {dot(temp2) = tempf+((-l)*(timef-t*(-s)))});
