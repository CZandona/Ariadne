...
inline HybridAutomaton getValve()
{
    ...
    //valve_automaton.new_mode(valve|modulated,{dot(aperture1)=K*(temp1-temp2), let(aperture2)= (1-aperture1)});
    valve_automaton.new_mode(valve|modulated,{dot(aperture1)=K*(temp1-temp2), dot(aperture2)= (-K)*(temp1-temp2)});
    ...
    valve_automaton.new_transition(valve|firer2,fire_room1,valve|modulated,{next(aperture1)=aperture1,next(aperture2)=aperture2},temp1>=tempfire,EventKind::URGENT);
    valve_automaton.new_transition(valve|modulated,closing,valve|end,{next(aperture1)=aperture1,next(aperture2)=aperture2},temp1<tempmin,EventKind::URGENT);
