...
inline HybridAutomaton create_heating_system()
{
    ...
    RealConstant h("h",4.3_decimal); // costante per coolgoon no valvola
    //RealConstant h("h",5.5_decimal); // costante per coolgoon
    ...
    //hr.new_mode( heating|start, {dot(temp) = v*pow(t,e))+m} );
    hr.new_mode( heating|start, {dot(temp) = 154*pow(t,Nat(0.25))+20} );
    hr.new_mode( heating|end, {dot(temp) = a*((log(b*(t-d)+c))/log(r))+d} );
    hr.new_mode( heating|coolon, {dot(temp) = tempf+((-g)*(1-aperture2)*(timef-(t*(-i))))});
